...
> **本 skill 不随 npm 包发布**。位置在 `.agents/skill-audit/`，项目级（开发者用），最终用户看不到。
...
```bash
# 审计单个工具
python3 .agents/skill-audit/scripts/audit.py check law-news

# 审计全部
python3 .agents/skill-audit/scripts/audit.py check-all

# 列出所有工具
python3 .agents/skill-audit/scripts/audit.py list-tools
```
